<?php
    ...
    class MagicToolboxCoreInstallerClass {
        ...
        function log($message) {
            ...
            $logFile = $this->logFileDir."/magiczoomplus_install.log";
            ...
            if($this->checkPlace() && $this->prepare()) {
                ...
                if($this->logEnabled && $this->installMode == 'install') {
                    if(!$this->isWriteable($this->logFileDir)) {/*is_writeable*/
                        ...
                        $canContinue = false;
                    } else if(file_exists($this->logFileDir."/magiczoomplus_install.log")) {
            ...
            switch($this->getStatus()) {
                case 'install':
                    switch($this->getStatus(true)) {
                        ...
                        case 'files':
                            if($this->logEnabled && file_exists($this->logFileDir."/magiczoomplus_install.log")) {
            ...
            if($dir = @opendir($src)) {
                while (($file = readdir($dir))!==false) {
                    ...
                    if(is_dir($src . '/' . $file)) {
                        ...
                        if($file == 'magiczoomplus.settings.dat') {
                            $file = 'magiczoomplus.settings.ini';
                            $fileExists = file_exists($dest.'/'.$file);
                            if($fileExists && !$overwrite) continue;
                            if(copy($src . '/magiczoomplus.settings.dat', $dest . '/' . $file)) {
                                if(!$fileExists) $this->log('CREATE FILE '.$dest.'/'.$file);
                            }
                        } else if(($file == 'magiczoomplus.css' || $file == 'magicscroll.css') && $this->resDir != '') {
                            $fileExists = file_exists($dest.'/'.$file);
                            if($fileExists && !$overwrite) continue;
        ...
        function uninstall_from_logFile($exclude = array()) {
            $errorLevel = 0;
            $lines = file($this->logFileDir."/magiczoomplus_install.log", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                    ...
                    case 'CREATE':
                        switch($matches[2]) {
                            ...
                            case 'DIR':
                                if(file_exists($this->dir.'/'.$matches[3]) && !in_array($matches[3], $exclude)) {
                                    if(!$this->isEmptyDir($this->dir.'/'.$matches[3])) {
                                        ...
                                        if(preg_match('/^magiczoomplus$/i', basename($matches[3]))) {
                            ...
                            default: //unknown command
                                $errorLevel = 2;
                                $this->setError('Wrong file format. The file magiczoomplus_install.log maybe was corrupted.');
                        }
                    break;
                    case 'CACHE':
                        switch($matches[2]) {
            ...
            if($errorLevel < 2) unlink($this->logFileDir."/magiczoomplus_install.log");
        ...
        function sendStat($mode = '') {
            ...
            if('working' == 'v4.9.21' || 'working' == 'v4.5.41') {
                return;
            }

            $hostname = 'www.magictoolbox.com';
            $url = $_SERVER['HTTP_HOST'].preg_replace('/\/\magiczoomplus\/installer\.php.*?$/i', '', $_SERVER['REQUEST_URI']);
            $url = urlencode(urldecode($url));
            $platformVersion = $this->getPlatformVersion();
            $path = "api/stat/?action={$mode}&tool_name=magiczoomplus&license=trial&tool_version=v4.5.41&module_version=v4.9.21&platform_name=magento&platform_version={$platformVersion}&url={$url}";
